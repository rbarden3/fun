...
def minimize_distances(graph, grid_positions, max_iterations=100):
    """Adjust grid positions to minimize Euclidean distance between connected nodes."""
    grid = generate_grid(grid_positions)
    grid = remove_empty_rows(grid)
    grid_positions = get_grid_positions(grid)

    for _ in range(max_iterations):
        updated_positions = grid_positions.copy()
        grid = generate_grid(updated_positions)
        grid = remove_empty_rows(grid)
        updated_positions = get_grid_positions(grid)
        for node in graph.nodes:
            connected_nodes = list(graph.neighbors(node))
            if not connected_nodes:
                continue

            # Compute the average position of connected nodes
            avg_x = np.mean(
                [grid_positions[neighbor][0] for neighbor in connected_nodes]
            )
            avg_y = np.mean(
                [grid_positions[neighbor][1] for neighbor in connected_nodes]
            )

            # Move the node closer to the average position
            curr_x, curr_y = grid_positions[node]
            new_x = round(curr_x + 0.5 * (avg_x - curr_x))
            new_y = round(curr_y + 0.5 * (avg_y - curr_y))

            # Avoid overwriting positions
            if (new_x, new_y) not in updated_positions.values():
                updated_positions[node] = (new_x, new_y)
            elif (
                abs(avg_x - curr_x) > abs(avg_y - curr_y)
                and (new_x, curr_y) not in updated_positions.values()
            ):
                updated_positions[node] = (new_x, curr_y)
            elif (curr_x, new_y) not in updated_positions.values():
                updated_positions[node] = (curr_x, new_y)

        # Update grid positions
        grid_positions = updated_positions
    return grid_positions
...
# initial_positions = nx.nx_pydot.pydot_layout(G, prog="circo")
